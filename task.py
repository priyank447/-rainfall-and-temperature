...
print(df_temp_f)
#df_temp_f.plot.scatter(x='Year', y='Temperature', label='Temperature andYear')
#plt.show()
df_rain_f = df_rain.query('Rainfall < 6 & Rainfall > 0')
#df_temp_f.plot.scatter(x='Year', y='Rainfall', label='Rainfall andYear')

#plt.show()

df_merge= pd.merge(df_temp, df_rain_f, on='Year', how='inner')
